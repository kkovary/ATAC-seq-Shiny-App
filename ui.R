...
ui <- navbarPage(
  ...
  tabPanel(
    'Introduction',
    shiny::tags$head(includeHTML((
      'HTML/googleAnalytics.html'
    ))),
  ...
  tabPanel(
    ...
    sidebarPanel(
      ...
      h4('Peak-gene links'),
      h6(
        'We defined putative enhancer-gene linkages on the basis of the correlation of accessibility and gene expression across cell lineages. These parameters filter the data displayed by features of those correlations.'
      ),
      selectInput(
        'transcriptID',
      ...
      h6(
        'Peaks containing the selected motifs will be displayed on the ATAC-seq plot.'
      ),
    ),
    mainPanel(width = 10,
